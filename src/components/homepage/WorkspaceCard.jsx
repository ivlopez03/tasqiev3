...
function WorkspaceCard({workspace,workspaces}) {
  ...
  return (
    <div className={`group relative flex items-center p-3 w-[270px] min-w-[270px] border border-transparent rounded-md ${cardVisibility ? 'border-[#e5e7eb]' : ''} hover:border-[#e5e7eb] transition duration-300 `}>
      ...
      >
        <span className={ `font-bold ${workspace.background_color === '#fff232' ?  'text-black' : 'text-white' }`}>{workspace.workspace_title[0] && workspace.workspace_title[0].toUpperCase()}</span>
      </div>
      <div>
        <Link to={`/workspace/${workspace.id}`}>
            <div className="text-md">{workspace.workspace_title}</div>
        </Link>
        <div className='text-[11px] text-gray-400'>Last activity:{workspace.updated_at}</div>
      </div>
      <div className={`absolute right-2 top-2 ${cardVisibility ? 'visible' : 'invisible'} group-hover:visible `}>
        <div className="flex items-center justify-center">
            ...
            {isSubmenuOpen && (
                <div ref={submenuRef} className="absolute top-3 right-0 bg-white border border-gray-200 rounded-md shadow-md z-10">
                    <ul className="text-[11px]">
                        ...
                        <li onClick={toggleDeleteModal} className="py-1 px-3 rounded-b-md cursor-pointer hover:bg-base200 text-[tomato]">Delete</li>
                    </ul>
